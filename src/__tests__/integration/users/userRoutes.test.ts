...
describe("/users", () => {
    ...
    test("GET /users -  Must be able to list users",async () => {
        ...
        const response = await request(app).get('/users').set("Authorization", `${adminLoginResponse.body.token}`)
    ...
    test("GET /users -  should not be able to list users not being admin",async () => {
        ...
        const response = await request(app).get('/users').set("Authorization", `${userLoginResponse.body.token}`)
    ...
    test("DELETE /users/:id -  should not be able to delete user without authentication",async () => {
        const adminLoginResponse = await request(app).post("/login").send(mockedAdminLogin);
        const UserTobeDeleted = await request(app).get('/users').set("Authorization", `${adminLoginResponse.body.token}`)
    ...
    test("DELETE /users/:id -  should not be able to delete user not being admin",async () => {
        ...
        const response = await request(app).delete(`/users/${UserTobeDeleted.body[0].id}`).set("Authorization", `${userLoginResponse.body.token}`)
    ...
    test("DELETE /users/:id -  Must be able to soft delete user",async () => {
        ...
        const response = await request(app).delete(`/users/${UserTobeDeleted.body[0].id}`).set("Authorization", `${adminLoginResponse.body.token}`)
        const findUser = await request(app).get('/users').set("Authorization", `${adminLoginResponse.body.token}`)
        expect(response.status).toBe(400)
    ...
    test("DELETE -  should not be able to delete user with invalid id",async () => {
        ...
        const response = await request(app).delete(`/users/13970660-5dbe-423a-9a9d-5c23b37943cf`).set("Authorization", `${adminLoginResponse.body.token}`)

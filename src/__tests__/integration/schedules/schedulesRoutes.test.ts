...
import app from "../../../app";
import {mockedUser, mockedAdmin, mockedAdminLogin, mockedCategory, mockedProperty, mockedUserLogin, mockedSchedule, mockedScheduleInvalidPropertyId, mockedScheduleInvalidDate, mockedScheduleInvalidHour} from "../../mocks"
...
describe("/schedules", () => {
    ...
    beforeAll(async() => {
        ...
        const categories = await request(app).post('/categories').set("Authorization", `${adminLoginResponse.body.token}`).send(mockedCategory)
        mockedProperty.categoryId = categories.body.id
        await request(app).post('/properties').set("Authorization", `${adminLoginResponse.body.token}`).send(mockedProperty)
    })
    ...
    test("POST /schedules -  should be able to create a schedule",async () => {
        const adminLoginResponse = await request(app).post("/login").send(mockedAdminLogin);
        const users = await request(app).get('/users').set("Authorization", `${adminLoginResponse.body.token}`)
        ...
        const response = await request(app).post('/schedules').set("Authorization", `${userLoginResponse.body.token}`).send(mockedSchedule)
    ...
    //     const response = await request(app).post('/schedules').set("Authorization", `${userLoginResponse.body.token}`).send(mockedScheduleInvalidDate)
    ...
    test("POST /schedules -  should not be able to create a schedule with an invalid hour",async () => {
        ...
        const response = await request(app).post('/schedules').set("Authorization", `${userLoginResponse.body.token}`).send(mockedScheduleInvalidHour)
    ...
    test("POST /schedules -  should not be able to create a schedule with an invalid property id",async () => {
        ...
        const response = await request(app).post('/schedules').set("Authorization", `${userLoginResponse.body.token}`).send(mockedScheduleInvalidPropertyId)
    ...
    test("GET /schedules/properties/:id -  must be able to list the schedules of a property",async () => {
        const adminLoginResponse = await request(app).post("/login").send(mockedAdminLogin);
        const properties = await request(app).get('/properties')
        const response = await request(app).get(`/schedules/properties/${properties.body[0].id}`).set("Authorization", `${adminLoginResponse.body.token}`)
    ...
    test("GET /schedules/properties/:id -  should not be able to list the schedules of a property with invalid id",async () => {
        const adminLoginResponse = await request(app).post("/login").send(mockedAdminLogin);
        const response = await request(app).get(`/schedules/properties/b855d86b-d4c9-41cd-ab98-d7fa734c6ce4`).set("Authorization", `${adminLoginResponse.body.token}`)
    ...
    test("GET /schedules/properties/:id -  should not be able to list the schedules of a property that the user is not admin",async () => {
        const userLoginResponse = await request(app).post("/login").send(mockedUserLogin);
        const properties = await request(app).get('/properties')
        const response = await request(app).get(`/schedules/properties/${properties.body[0].id}`).set("Authorization", `${userLoginResponse.body.token}`)
